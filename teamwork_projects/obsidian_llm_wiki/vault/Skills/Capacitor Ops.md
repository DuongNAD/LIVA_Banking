...
## Description
Core guidelines and diagnostic protocols for managing the Capacitor 8 native Android application, versionCode calculation, intent share targets, and native OAuth.
...
Adhere to these strict Capacitor and Android platform rules:
...
## Verification Method
1. Verify version code calculations match the `major × 10000 + minor × 100 + patch` scheme.
2. Ensure no direct references to `CapacitorShareTarget.removeAllListeners()` or `Capacitor.isNativePlatform()` exist in the codebase.
